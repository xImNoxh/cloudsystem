...
import de.polocloud.modules.smartproxy.pluginside.BungeeBootstrap;
...
public class PlayerInjectListener implements Listener {
    ...
    private void injectConnection(Connection connection) {
        if (BungeeBootstrap.getInstance().getAddresses().get(connection.getAddress()) == null) return;
        try {
            ...
            addressField.set(wrapper, BungeeBootstrap.getInstance().getAddresses().get(connection.getAddress()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
